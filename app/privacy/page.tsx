...
export const metadata: Metadata = {
  ...
  description:
    "MotionAnalysys Start無料ベータ版におけるスタート動画、選手区分、校正、分析結果の取扱いについて説明します。",
};
...
export default function PrivacyPage() {
  return (
    <LegalPage
      title="プライバシーポリシー"
      description="MotionAnalysys Startは、利用者が選んだ競泳スタート動画をブラウザ内で処理する無料ベータ版のローカル分析ツールです。ここでは、分析時に扱う情報と、その保存・送信について説明します。"
      lastUpdated="2026年9月9日"
    >
      <LegalSection id="local-processing" title="1. 映像は端末内で処理します">
        <p>
          利用者が端末から明示的に選択したスタート動画、姿勢ランドマーク、自動判定とコーチ確認によるイベント、校正値、分析結果は、利用中のブラウザ内で処理します。本アプリには、動画または分析結果を運営者のサーバーへアップロードする機能はありません。無料ベータ版はライブカメラ入力を使用しません。
        </p>
        <p>
      ...
      <LegalSection id="data" title="2. 本アプリが扱う情報">
        <ul>
          ...
          <li>
            <strong>種目と校正プロファイル：</strong>
            選択した種目と0m・5m・水面・進行方向の設定を、同じ撮影条件で比較しやすくするためにブラウザ内で使用します。校正プロファイルは
            <code>localStorage</code>
            に保存されます。
          </li>
          <li>
            <strong>年齢と研究比較区分：</strong>
            解析対象とBorn et al. (2026)の参考帯表示を決めるため、現在のページ内だけで使用します。運営者のサーバーにも
            <code>localStorage</code>
            にも保存しません。利用者が結果を書き出した場合だけ、再現情報としてそのファイルに含まれます。
          </li>
          <li>
            <strong>分析結果：</strong>
            姿勢ランドマーク、自動判定またはコーチ確認によるスタートイベント、判定ポリシーとスコア、時間、2D距離・速度・角度、研究参考帯、品質情報、修正履歴を現在のセッション内で生成します。
          </li>
          <li>
      ...
      <LegalSection id="not-collected" title="3. 収集しない情報">
        <p>
          本アプリは、アカウント登録、クラウド履歴、映像・分析結果・年齢・研究比較区分のサーバー保存、広告トラッキングを提供していません。映像、姿勢ランドマーク、校正プロファイル、分析値を外部へ送信する処理も実装していません。
        </p>
        <p>
      ...
      <LegalSection id="local-storage" title="4. 動画選択とブラウザ保存">
        ...
        <p>
          年齢と研究比較区分は校正プロファイルに含めず、ページの再読み込みで破棄します。これらを含むJSON、CSV、PNGを利用者が保存した後は、利用者自身で管理してください。
        </p>
      </LegalSection>
