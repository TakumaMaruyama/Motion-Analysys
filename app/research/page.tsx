    ...
    >
      <LegalSection id="principles" title="1. 何を重視するか">
        <ul>
          ...
          <li>自動検出は候補提示に限定し、コーチが確認したイベントだけを指標計算に使います。</li>
      ...
      <LegalSection id="reference-band" title="2. エリート参考帯の読み方">
        ...
        <p>
          表示は「P25〜P50」のような位置の説明だけです。A〜E評価、合否、総合点、才能判定は行いません。研究対象範囲に合わせ、13〜32歳だけに参考帯を表示します。33歳以上は解析できますが、帯は表示しません。
        </p>
        <p>
      ...
      <LegalSection id="measurement" title="3. 測定値の位置づけ">
        <p>
          簡易タイムモードは初動、ブロック／壁接触、押し出し、飛行、入水、5mの時間だけを参考計測します。距離、速度、角度、0〜5m平均速度は算出しません。
        </p>
        <p>
          ブロック／壁接触時間と、定義が一致する5m時間は比較しやすい指標です。一方、初動、入水イベント、角度、2D速度、入水距離には撮影方向、フレームレート、校正、ランドマーク定義による差が含まれます。画面外、遮蔽、飛沫、複数人物、カメラ移動、低信頼度がある場合は数値を
          <code>null</code>
          とし、0で置き換えません。
        </p>
        <p>
