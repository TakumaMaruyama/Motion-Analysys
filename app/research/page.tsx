  ...
  return (
    ...
    >
      ...
      <LegalSection id="measurement" title="3. 測定値の位置づけ">
        <p>
          簡易タイムモードは初動、ブロック／壁接触、押し出し、飛行、入水、5mの時間に加え、0m・5mの2点校正から離台直後・入水直前の前方速度を低精度2D推定として表示します。初動・離台・飛行は条件を満たせば自動判定し、入水・5mは手動確認を前提とします。簡易モードの速度は30fps、斜め撮影、遠近、カメラ移動の影響を強く受けるため、精密モードの速度や研究値と同等には比較できません。入水距離、角度、0〜5m平均速度、研究百分位は算出しません。
        </p>
        <p>
