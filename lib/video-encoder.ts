  ...
  static async encodeToMP4(
    ...
    try {
      ...
      if (support.supported) {
        return this.encodeFramesToVideo(
          frames, 
          { ...options, codec: 'avc1.42001E' },
          onProgress
        );
      } else {
        // フォールバックとしてWebMを使用
        return this.encodeFramesToVideo(frames, options, onProgress);
      }
    } catch (e) {
      // エラー時はWebMにフォールバック
      return this.encodeFramesToVideo(frames, options, onProgress);
