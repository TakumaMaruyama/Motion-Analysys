...
- 簡易タイム：30fps以上、斜め撮影可。時間6指標だけを参考計測
...
2. 精密では0m、5m、水面の2点、進行方向を校正する。簡易タイムでは進行方向だけを選ぶ。
3. 信号、初動、手の離台、後足の離台、離台、頭頂入水、任意の5m通過を
   1フレーム単位で確認する。背泳ぎでは飛び込み専用イベントを除く。
4. 確認済みイベントだけから局面別結果、品質・制約、JSON／CSV／PNGを表示・保存する。
   研究参考帯は精密モードだけに表示する。
...
簡易タイムでは、初動、ブロック／壁接触、押し出し、飛行、入水、5mの
時間6指標だけを表示します。入水距離、両前方速度、体幹角度、0〜5m平均速度、
研究参考帯は計算・表示しません。精密モードは下表の全指標を対象にします。
...
このリポジトリはSites/Vinext向け設定を含みます。GitHub push、Sites公開、
本番動作確認はそれぞれ区別して実施・記録します。
