...
固定したプールサイドの水上スマートフォン1台で撮影した側面動画から、
競泳スタートの離台・飛行・入水を局面別に確認する、ブラウザ内処理の
未検証ベータです。
...
撮影条件は、位置・高さ・向き・ズームを固定した側面映像、1レーン・1人、
0m壁・5m位置・水面が同時に見える画角です。最低60fps、120fpsを推奨します。

## 4段階ワークフロー

1. 種目、年齢、研究比較区分、動画を選ぶ。
2. 映像上で0m、5m、水面の2点、進行方向を校正する。
3. 信号、初動、手の離台、後足の離台、離台、頭頂入水、任意の5m通過を
   1フレーム単位で確認する。背泳ぎでは飛び込み専用イベントを除く。
4. 確認済みイベントだけから局面別結果、研究参考帯、品質・制約、
   JSON／CSV／PNGを表示・保存する。

自動解析は候補提示までです。`candidate`または`needs-review`のイベントは
計算依存関係に使用せず、コーチが映像で確認した`verified`イベントだけを
使います。画面外、遮蔽、飛沫、低信頼度、不成立は`null`で表し、0で代用しません。
...
- [`lib/start/analysis.ts`](lib/start/analysis.ts) — verified依存、時間・2D速度・角度、品質、修正履歴
...
公開時点の表示は「未検証ベータ」です。変更には以下の外部証拠が必要です。
...
このリポジトリはSites/Vinext向け設定を含みます。実装、GitHub push、Sites公開、
本番動作確認は別の段階です。この変更ではデプロイを行いません。
