...
5. **確認・修正・保存**
   ...
   タイムライン上でゲート通過とストロークマーカーを追加・削除・移動し、再計算後の結果をJSON、CSV、PNGで保存します。公式記録としては使用できません。
...
元動画から回転、解像度、実効fps、フレーム時刻、コーデック可否を確認します。WebCodecsまたはコーデックが利用できない場合はプレビューまでとし、精密解析は開始しません。1回のSwim解析区間は30秒以内、30fps以上（60fps推奨）です。
...
- 固定カメラで撮影した1レーン・1人の動画を基本とします。カメラが動く、ズームが変わる、複数人が重なる場合、校正条件と計測結果は信頼できません。
